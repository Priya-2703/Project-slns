...
const NewArrival = () => {
  ...
  // ✅ Get latest 10 products (sorted by created_at or updated_at)
  const arrivalProducts = React.useMemo(() => {
    if (!data || data.length === 0) return [];

    // Sort by created_at (or updated_at) in descending order (newest first)
  ...
  // Don't render if no arrival products
