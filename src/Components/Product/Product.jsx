...
const Product = () => {
  ...
  const handleSearch = (e) => {
    e.preventDefault();
    // just trigger filter by updating state — displayProducts auto update aaguthu
    setProducts(data); // reset to original data
  };
        ...
        <div className="w-[90%] flex justify-between items-center mx-auto mt-4 py-3">
          ...
          <div className="flex justify-center items-center w-[200px] md:w-[300px] gap-1 md:gap-4 border border-[#955E30] overflow-hidden rounded-l-full">
            <div className="flex justify-center items-center relative px-2 ">
              ...
              />
              {search && (
                <button onClick={clearSearch} className="absolute right-0 md:-right-3">
                  <X color="white" size={mobileView? 14 :20}/>
                </button>
              )}
            </div>
            <div
              className="bg-[#955E30] px-3 h-full py-2"
          ...
          <div className="flex justify-center items-center gap-4 lg:gap-8">
            {categories.map((item) => (
              ...
              >
                ...
                >
                  <img src={item.img} alt="category" className="object-cover" />
                </div>
                <h1 className="text-[8px] md:text-[10px] text-center font-['Poppins']">{item.name}</h1>
              </div>
            ))}
          </div>
